...
- **後端 (Python)**：
  ...
  - 呼叫 `src/pipeline.py` 串接 Advisor 核心邏輯，並依市場採用各自最適策略：
    - **台股**：五因子（趨勢／動能／量能／三大法人籌碼／月營收）＋大盤多空動態門檻。曝險水位採用專屬的目標波動率 `VOL_TARGET_TW=20%`（適應台股長期較高的波動性，中位數約 16%）。
    - **美股**：橫斷面動能策略（`src/advisor/us_screener.py`）——以「波動率調整的 12-1 月相對動能」對**完整 S&P 500 成分股**排名，僅保留站上 200 日線者。曝險水位採用 `VOL_TARGET_US=15%`（適應美股較低的波動性，中位數約 12%）。此策略經 `backtest_us.py` 回測驗證，是台股五因子直接移植美股失效後的替代方案。
...
針對不喜歡頻繁換股、只想長抱大盤的投資人，系統也內建了核心 ETF 專屬的「存股紅綠燈」。

**追蹤標的**：0050.TW (台灣50)、0056.TW (高股息)、VOO (S&P 500)、QQQ (Nasdaq 100)、SMH / SOXQ (半導體)。

**進出場邏輯 (無需看盤，每天看燈號即可)**：
- 🟢 **多頭續航 (價格 > 50MA > 200MA)**：強勢多頭，繼續定期定額。入手參考價為 **50MA**。
- 🟡 **回檔買點 (200MA < 價格 < 50MA)**：長期趨勢未破，但短期拉回。離 200MA 停損點極近，是**絕佳的單筆加碼點**！
- 🔴 **空頭觀望 (價格 < 200MA)**：跌破長期生命線，趨勢轉弱。**200MA 即為出場防守底線**，跌破建議轉移資金至避險資產，站回再買。
- *註：ETF 管理費會直接反映在淨值與收盤價的扣除中，因此看技術線圖即可，無需額外扣除管理費成本。*

### 📲 ETF 與大盤進出場 LINE 每日自動警報
透過 `etf_alert.py`，系統每天跑完日更後，會自動對比前一日的狀態。當大盤風險變化，或是核心 ETF 發生突破/跌破均線與 KD 交叉時，會主動推播 LINE 給您：
- ⚠️ **大盤風險警報**：波動率上升導致建議曝險水位下降時，提醒您優先停利換取現金。
- 🟢 **轉強加碼提醒**：ETF 重新站回 50 日均線（洗盤結束）時，推播買點與防守價。
- 🔴 **破線停損警報**：ETF 跌破 200 日均線時，強烈提醒停損。
- 🔀 **KD 短線動能警報**：基於 5 年歷史回測的數據驅動提醒，破除傳統「超買必跌、超賣必漲」的迷思：
  - **黃金交叉**：低檔區 (20~50) 勝率最高 (>60%) 為絕佳加碼點；超賣區 (<20) 通常為跌深反彈，勝率僅約 50%。
  - **死亡交叉**：強勢多頭常在超買區 (>80) 鈍化，死叉後續跌機率極低（<30%），提醒勿盲目殺低；高檔區 (50~80) 死叉則需留意短線回檔。
