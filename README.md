...
每週六早上 10:00，系統會自動在 **GitHub Actions** 上執行覆盤程式，比對當週選股與最新收盤價。

**主 KPI**：每日 Top3 合集的命中率 **P**，對同一週股票池上漲比例 **U**。超額 P−U > 0 代表有抓到相對強勢（盤差時池子只有 20% 上漲、Top3 命中 45% 算過關）。合集（一週 Top30 去重）只當附註。警報改看連續週是否 **P≤U**，不再用絕對勝率 50%。
